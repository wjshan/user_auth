...
- 🎯 Entry points to execute your program using `python -m <user_auth>` or `$ user_auth` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/wjshan/user_auth/branch/main/graph/badge.svg?token=user_auth_token_here)](https://codecov.io/gh/wjshan/user_auth)
[![CI](https://github.com/wjshan/user_auth/actions/workflows/main.yml/badge.svg)](https://github.com/wjshan/user_auth/actions/workflows/main.yml)

Awesome user_auth created by wjshan

## Install

from source
```bash
git clone https://github.com/wjshan/user_auth user_auth
cd user_auth
make install
```
...
```bash
pip install user_auth
```

## Executing

```bash
$ user_auth run --port 8080
```

or

```bash
python -m user_auth run --port 8080
```

or

```bash
$ uvicorn user_auth:app
```
...
```bash
❯ user_auth --help
Usage: user_auth [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ user_auth create-user --help
Usage: user_auth create-user [OPTIONS] USERNAME PASSWORD
...
```bash
user_auth create-user admin admin --superuser
```
...
Run with `user_auth run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `user_auth create-user`.
...
-----------------------------------------------------
user_auth/__init__.py              4      0   100%
user_auth/app.py                  16      1    94%
user_auth/cli.py                  21      0   100%
user_auth/config.py                5      0   100%
user_auth/db.py                   10      0   100%
user_auth/models/__init__.py       0      0   100%
user_auth/models/content.py       47      1    98%
user_auth/routes/__init__.py      11      0   100%
user_auth/routes/content.py       52     25    52%
user_auth/routes/security.py      15      1    93%
user_auth/routes/user.py          52     26    50%
user_auth/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in user_auth/default.toml.
